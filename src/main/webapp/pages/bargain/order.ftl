...
.pronotice img{ position: absolute; top: 22px; left: 50%; width: 20px; margin-left: -10px}
	...
	<body>
           ...
           <#if addressId?exists && addressId!='' &&addressId !='-1' >
             <#if defaultAddress?? && ( defaultAddress.id >0 ) >
                 <#assign  da=defaultAddress />
						<a href="${rc.contextPath}/bargain_ad/listorderads/${(ordertype)!"1"}/${(isBonded)!"0"}">
			   ...
			   <#else>
					<a href="${rc.contextPath}/bargain_ad/toorderaddads/${(ordertype)!"1"}/${(isBonded)!"0"}">
			...
			<div class="paytype">
			<#assign cutPrice = point?number/100 />
			
				<div class="choosepay">选择支付方式<span>合计金额：<em class="payPrice">${allTotalPrice!"0.00"}</em></span></div>
				<div class="paytb" id="paywexin"><img src="${rc.contextPath}/pages/images/wxpay.png" class="cpay">
				微信支付
				
				<img src="${rc.contextPath}/pages/images/<#if iswx5version?exists && (iswx5version == '1')>select.png<#else>unselect.png</#if>" class="cpayflag"></span></div>
			</div>
			<div class="pay clear">
				<ul>
					<li>微信支付</li>
				</ul>
				总计￥<span class="payPrice priceEm">${allTotalPrice!"0.00"}</span>
		...
		<script>
			$(function(){
				var totalPrice = $('#point2cash').attr('totalPrice');
				var point = $('#point2cash').attr('point');
				if(totalPrice > point/100){
					cutPrice = point/100;
				}
				else{
					cutPrice = totalPrice-0.01;
				}
				$('#checkbox').val(cutPrice);
				$('#cutPrice').html(cutPrice);
				$('#point2cash').click(function(){
					if($('.priceEm').html()==totalPrice){
						var price = totalPrice-cutPrice;
						$('.priceEm').html(price.toFixed(2));
					}
					else{
						$('.priceEm').html(totalPrice);
					}
				...
				$('#topayId').click(function(){
				   ...
				   $('.pronotice').hide();
				      ...
				      if( document.getElementById('addressIdCard')!=undefined && document.getElementById('addressIdCard')!=null    &&  addressIdCard =='')
				      {
				          ...
				          document.modifyidcardForm.action="${rc.contextPath}/bargain_ad/toordermdfads/${(addressId)!"-1"}/${(ordertype)!"1"}/${(isBonded)!"0"}";
				      ...
				      }else if(paytype !=undefined && paytype=='3')
					  {
					      ...
					      var requesturl ="${rc.contextPath}/bargain/addajax"
						        ...
						        var redirecturl = msgjsonobj.redirecturl ;
							       ...
							       if(addoderstatus !=undefined && addoderstatus == '1')
							       {
							          showTipLoadingMsg();
								          ...
								          var wxtopayurl = "${rc.contextPath}"+redirecturl; /* /bargin_order/topaywx/3 */
								          var usePoint = "0";
									        if($('#point2cash').is(':checked')){
									        	usePoint = "1";
									        }
									        else{
								          ...
								          var wxtopayreqdata = "accountId="+accountId+"&orderIdList="+orderIdList+"&usePoint="+usePoint  ;
