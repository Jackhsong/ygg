  	    		 ...
  	    		 success:function(dataX){
  	    		         ...
  	    		         if(data1.length > 0)
		    			  {
		    				  ...
		    				  for(var j = 0 ; j <= _status.length ; j++){
		    					  var _statusVal = _status.eq(j).attr("value");
		    					  if(_statusVal == 1){
		    						  _status.eq(j).html("预期奖励");//html("未付款"); 
		    					  }else if(_statusVal == 2){
		    						  _status.eq(j).html("预期奖励");//html("已付款");
		    					  }else if(_statusVal == 3){
		    						  _status.eq(j).html("预期奖励");//html("已发货"); 
		    					  }else if(_statusVal == 4){
		    						  _status.eq(j).html("预期奖励");//html("可提现"); 
		    						  _status.eq(j).parent().find("h3")
		    					  }
		    					  else if(_statusVal == 5){
		    						  _status.eq(j).html("可提现"); 
		    						  _status.eq(j).parent().find("h3")
		    					  }
		    					  else if(_statusVal == 6){
		    						  _status.eq(j).html("部分提现"); 
		    						  _status.eq(j).parent().find("h3")
		    					  }
		    					  else if(_statusVal == 7){		    					  		    				
		    						  _status.eq(j).html("已提现"); 
		    						  _status.eq(j).parent().find("h3")
		    					  }
		    					  else if(_statusVal == 8){		    					  
		    						  _status.eq(j).html("已到账"); 
		    						  _status.eq(j).parent().find("h3")
		    					  }		    					  
		    				  }  
