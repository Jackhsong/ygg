...
public class DoInNewTranscationServiceImpl implements DoInNewTranscationService
{
    ...
    public void autoConfirmReceiptInNewTranscation(Map<String, Object> map)
    {
      ...
      //修改粉丝订单状态为可提现
        try
        {
        	hqbsFansOrderDao.updateStatus(orderId,accountId,(byte)2);
        } catch (Exception e){
            log.error("autoConfirmReceiptInNewTranscation方法中执行————>updateStatus失败,orderId：" + orderId + ",accountId:" + accountId+",status:2");
        }
        log.info("updateStatus通过————>orderId："+orderId+",accountId:"+accountId);
