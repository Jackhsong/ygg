...
/**
 * 拦截器寂寞授权后获取微信的相关信息 1.用户为新用户时，添加账号信息包括：基本信息表：account 心动慈露用户表：qqbs_account 心动慈露粉丝关系表：qqbs_fans 2.为老用户是，更新微信头像
 * 
...
//                            return "redirect:" + "/error/guide";
                        ...
                        try
                        {
                        	String prefix = "activity_b?accountId=";
                        	
                            int a1 = state.indexOf(prefix);
                            if (a1 != -1)
                            {
                                int a2 = state.indexOf("&");
                                String taccountId = "";
                                if (a2 != -1)
                                {
                                    taccountId = state.substring(a1 + prefix.length(), a2);
                                }
                                else
                                {
                                    taccountId = state.substring(a1 + prefix.length());
                                }
                                qqbsAccount.setFatherAccountId(Integer.valueOf(taccountId));
                            }
                            else
                            {
                                logger.error("用户点击砍价B-link--推荐人为空--不做处理--");
                            }
                        }
                        catch (Exception e)
                        {
                            logger.error("用户点击砍价B-link,转换推荐人ID出错", e);
                        }
