...
public class SpokesPersonServiceImpl implements SpokesPersonService {
	...
	public String getListInfo(QqbsAccountEntity account) {
		...
		try {
			...
			Map<String, Object> aPoint = accountDao.getPoint(param);
			point = Integer.parseInt(aPoint.get("point").toString());
		} catch (Exception e) {
		}

		result.addProperty("point", (point));
		...
		if (account.getFatherAccountId() == 0) {
			result.addProperty("recommendedPerson", "心动慈露");
		} else {
	...
	public String getQRCode(QqbsAccountEntity account, String appPath) throws Exception {
	...
	public String getQRCode(QqbsAccountEntity account) throws Exception {
		int accountId = account.getAccountId();

		if (isSpokenPerson(account)) {

			return QRCodeUtil.buildTemporaryQRCode(accountId + 1000000000);
		}
		return null;
	}
	...
	private String buildUpyunImgTarget(QqbsAccountEntity account) throws UnsupportedEncodingException {
	...
	private boolean isSpokenPerson(QqbsAccountEntity account) {
		return true;// (account.getSpokesPerson() == 1) ||
		            // getSpokesperson(account.getAccountId());
	}
	...
	public void updateFansListlinshi() {
		...
		for (QqbsFansOrderEntity oe : list) {
			...
			if (db == null) {
				...
				if (level == 1) {
					withdrawCash = (realPrice * 0.15f);
				} else if (level == 2) {
					withdrawCash = (realPrice * 0.2f);
				} else {
				...
				if (one != null) {
					if (ls.getFatherAccountId() == one.getFatherAccountId()) {
						...
						if (one.getFatherAccountId() == 0) {
							...
							// 1.心动慈露表中FatherAccountId=0，把历史数据插入qqbs_fans 里面处理订单
							processFansInfo(one, ls.getFatherAccountId());
						} else {
							log.error("accountId= " + ls.getAccountId() + "的FatherAccountId=" + ls.getFatherAccountId()
							        + "和qqbs_account表中FatherAccountId=" + one.getFatherAccountId() + "不相同,不处理");
						}
					}
				} else {
		...
		if (oneFans != null) {
			...
			if (oneFans.getFatherAccountId() != 0) {
				...
				if (secondFans != null) {
					...
					// {
					// QqbsAccountEntity thirdFans =
					// hqbsAccountDao.findAccountByAccountId(secondFans.getFatherAccountId());
					// if (thirdFans != null)
					// {
			...
			log.error(" accountId= " + one.getAccountId() + " FatherAccountId=" + one.getFatherAccountId()
			        + "在qqbs_account表中找不到");
		}
	}
	 ...
	 */
	private void processOrderInfo(List<QqbsFansOrderEntity> list, int level, QqbsAccountEntity one,
	        int fatherAccountId) {
		// c.处理订单
	...
	private String buildQRCodeImage(JsonObject json) {
		...
		try {
			...
			drawer.drawImage(qrCodeImage, Math.round((backgroundImage.getWidth() - 240) / 2), 350, 240, 240, null);
	...
	private BufferedImage loadFromURLOrFile(String urlOrFile) throws MalformedURLException, IOException {
