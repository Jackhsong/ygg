...
//    }
    ...
    public ModelAndView register(@RequestParam("accountid") String name, @RequestParam(value = "password", required = false, defaultValue = "") String password,
        ...
        @RequestParam(value = "sendnumberString", required = false, defaultValue = "") String sendnumberString)
            throws Exception
    {
        ModelAndView mv = new ModelAndView();
        ...
        */
        ...
        if(smsverify!=""&&smsverify!=null&&smsverify.equals(sendnumberString)){
	        mv.addObject("name", name);
	        mv.addObject("accountid", name);
	    	...
	    	mv.addObject(CommonConstant.ERROR_CODE, "验证码错误！");
	    	mv.setViewName("forward:/user/register"); // 失败还是在此页面
	        return mv;
	    } 
    }
	...
	@ResponseBody
	public String sendMessage(@RequestParam(value = "accountid", required = false, defaultValue = "") String mobile){
		ModelAndView mv = new ModelAndView();
		SDKTestSendTemplateSMS sdk=new SDKTestSendTemplateSMS();
		String sendnumberString=sdk.sendMessageToMobile(mobile);
	  
	    mv.addObject("sendnumberString",sendnumberString);
       ...
       String ip = CommonUtil.getRemoteIpAddr(request);
           ...
           if (mobileNumberCount > CommonConstant.SAME_NUMBER_SENDMSG_MAX_TIMES_PER_DAY)
            {
              ...
              jsonObj.addProperty("errorCode", CommonEnum.ACCOUNT_VERIFICATION_ERRORCODE.OUTOF_SENDMSG_TIMES.getValue());
               ...
               // 同一天内，同一个手机号发送短信请求次数达到上限时只发送一次
                if (isSend == null)
               {
                   CommonUtil.sendSMS(NOTIFY_MOBILE_NUMBER, "【心动慈露】手机号：" + accountid + "今日发送短信请求次数达到" + mobileNumberCount + "次，疑似恶意攻击，请处理", 5);
        ...
        else
        {
            ...
            if (IPCount > CommonConstant.SAME_IP_SENDMSG_MAX_TIMES_PER_DAY)
            {
                ...
                if (isSend == null)
                {
                    CommonUtil.sendSMS(NOTIFY_MOBILE_NUMBER, "【心动慈露】IP：" + ip + "今日发送短信请求次数达到" + IPCount + "次，疑似恶意攻击，请处理", 5);
