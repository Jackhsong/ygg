           ...
           <#elseif  orderStatus =='2'>
			...
			<div class="ordermain">
				...
				<#if orderProductList?exists && (orderProductList?size>0 ) >
					...
					<#if sellerName?contains("心动慈露")>
						${sellerName?replace("心动慈露","心动慈露")}
					<#else>
						杭州慈露有限公司
					</#if>
			...
			<div class="paytype">
				...
				<div class="paytb" id="paywexin"><img src="${rc.contextPath}/pages/images/wxpay.png" class="cpay">微信支付<span><span>+行动币抵扣(可抵扣<span id="cutPrice">${cutPrice}</span>元)</span><img src="${rc.contextPath}/pages/images/<#if iswx5version?exists && (iswx5version == '1')>select.png<#else>unselect.png</#if>" class="cpayflag"></span></div>
			</div>
			<#else>
			...
			<#if orderStatus?? && orderStatus =='1' >
			 ...
			 <div class="pay clear off">
				...
				<ul>
					<li id="paytitle">微信支付</li>
					<li><input  name="point2cash" type="checkbox" value="${cutPrice}" totalPrice="${realPrice}" point="${point}" id="point2cash"/>行动币抵现</li>
					<li></li>
				</ul>
				总计￥<span class="priceEm">${realPrice!"0.00"}</span>
				<#if endSecond?? &&  (endSecond=='-1' || endSecond =='0' ) >
